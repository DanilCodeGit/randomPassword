...
// объявляем необходимые глобальные переменные
var (
	lowerCharSet          = "abcdefghijklmnopqrstuvwxyz"
	upperCharSet          = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
	specialCharSet string = "!@#$%^&*()-"
	numberCharSet         = "012356789"
	minSpecialChar        = 2
	minUpperChar          = 3
	minNumberChar         = 3
	passwordLength        = 10
...
func generatePassword() string {
	...
	// генерация случайных спец.символов
	for i := 0; i < minSpecialChar; i++ {
		random := rand.Intn(len(specialCharSet))
		password = password + string(specialCharSet[random])
	}

	// генерация случайных символов в верхнем регистре
	...
	// считаем остаток для заполнения пароля символами в нижнем регистре
	...
	// генерация чисел в нижнем регистре
...
func main() {
	...
	// проверка соответствия длины пароля заданным критериям
	totalCharLenWithoutLowerChar := minUpperChar + minSpecialChar + minNumberChar
